...
exports.isLimit = function(sender, isPremium, isOwner, limitCount, _db){
    ...
    if (found === false) {
        ...
        _db.push(obj)
        fs.writeFileSync('./src/database/limit.json', JSON.stringify(_db, null, 2))
        return false
    }
}
...
exports.addBalance = function(sender, duit, _db){
    ...
    if (position !== false) {
        ...
        fs.writeFileSync('./src/database/balance.json', JSON.stringify(_db, null, 2))
    } else {
        const bulin = ({
            id: sender,
            balance: duit
...
exports.isGame = function(sender, isOwner, gcount, _db){
    ...
    if (found === false){
        ...
        _db.push(obj);
        fs.writeFileSync('./src/database/glimit.json',JSON.stringify(_db, null, 2));
        return false;
    }
}
...
exports.createHit = function(sender, _db){
    ...
    _db.push(anohoh);
    fs.writeFileSync('./src/database/userhit.json',JSON.stringify(_db, null, 2));''
