  ...
  axios.get('https://www.bmkg.go.id/gempabumi/gempabumi-dirasakan.bmkg').then((response) => {
    ...
    for (let i = 0; i < urlElems.length; i++) {
      ...
      if (urlSpan) {
        ...
        var rawWaktu = $(Kapan).html(); 
        var [tanggal, waktu] = rawWaktu.split('<br>'); 
        var bulanIndo = {
          '01': 'Januari',
          '02': 'Februari',
          '03': 'Maret',
        ...
        var formattedTanggal = `${hari} ${bulanIndo[bulan]} ${tahun}`;
        var Dirasakan = [];
        $(Wilayah).find('span.label-warning').each((index, element) => {
          Dirasakan.push($(element).text());
        });
