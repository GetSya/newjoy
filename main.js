...
getMessage: async (key) => {
   ...
   if (pairingCode && !client.authState.creds.registered) {
    if (useMobile) throw new Error('Tidak dapat menggunakan kode pairing dengan API Mobile');
    if (!!phoneNumber) {
        const sanitizedPhoneNumber = phoneNumber.replace(/[^0-9]/g, ''); 
        if (!/^\d{10,15}$/.test(sanitizedPhoneNumber)) {
            console.log(chalk.bgBlack(chalk.redBright("Nomor telepon di config.nobot tidak valid. Harus dimulai dengan kode negara, Contoh: +62xxx")));
            process.exit(0);
        }
    } else {
        console.log(chalk.bgBlack(chalk.redBright("Nomor telepon tidak ditemukan di config.nobot")));
        process.exit(0);
    }
    setTimeout(async () => {
        let code = await client.requestPairingCode(phoneNumber);
        code = code?.match(/.{1,4}/g)?.join("-") || code;
        console.log(chalk.bold.rgb(255, 136, 0)(`\n  ╭────────────────────────────╮\n  │  ${chalk.yellow('Kode Pairing Anda:')} ${chalk.greenBright(code)}  │\n  ╰────────────────────────────╯\n            `));
    }, 3000);
}
...
XeonLft = await getBuffer(ppuser)
                ...
                xeonbody = `Hallo @${xeonName.split("@")[0]}. Selamat datang di grup : ${metadata.subject}`
